...
const AdvanceComponent: FC<IAdvanceComponent> = ({ selectedElement }) => {
  ...
  const selectedContractAbi: string = useSelector((state: IRootState) => state.contract.contractDetails.abi);
  const selectedContractAddress: string = useSelector((state: IRootState) => state.contract.contractDetails.address);
    ...
    <>
      {selectedContractAbi && selectedContractAddress && selectedContractNetwork ? (
        <>
          ...
          <AbiMethods setShowComponent={setShowComponent} selectedElement={selectedElement} setMethodOpen={setMethodOpen} setIsOpen={setIsOpen} />
        ...
        <>
          ...
          <p className="contract-text ml-[0.5rem]">Integrate your Frontend with smart contracts.</p>
          <div className="flex items-center px-3 mt-5 mb-[2rem] text-black">
            <div onClick={() => setIsOpen(true)} className="flex cursor-pointer contract-button w-full py-2.5 pl-6 pr-7 ml-2">
              <span className="mt-1 ml-4 mr-3">
                <FaFileContract />
              </span>
              Import Contract
              <ContractModal isOpen={isOpen} setIsOpen={setIsOpen} methodOpen={methodOpen} setMethodOpen={setMethodOpen} />
            </div>
          </div>
          {selectedElement.connectWallet ? <WarningText text="Sorry! Connect wallet and contract binding cannot be for common button" /> : null}
          {isWalletConnect < 0 ? <WarningText text="Please Add connect wallet button before importing Contract" /> : null}

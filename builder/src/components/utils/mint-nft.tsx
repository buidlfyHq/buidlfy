...
const MintNFT: FC<IMintNFT> = ({
  ...
  const currentPublishStep = useSelector(
    (state: IRootState) => state.publish.currentStep
  );
  const isPublished = currentPublishStep < 6
    ...
    <>
        { currentAccount && 
            <Menu as="div" className="relative">
                <Menu.Button 
                as='div'
                onClick={!isPublished && handleMintTemplateForm }
                className={`${!isPublished ? 'cursor-pointer' : 'cursor-not-allowed' } py-3 px-5 my-2 ml-3 text-[14px] font[500] button-singleTemp`}
                >
                ...
                {isPublished ? (
                    <Menu.Items className="py-6 px-4 absolute right-0 top-[4rem] bg-white w-[214px] shadow-menu rounded-[16px]">
                        ...
                        >
                            ...
                            <button
                                className={`${!isNoElementinWorkspace ? "" : "opacity-30 pointer-events-none"} w-full py-2 px-auto mt-4 font-[500] text-[14px] text-white rounded-[4px] cursor-pointer connect-wallet-button whitespace-nowrap`}

...
const RenderTemplateList: FC<IRenderTemplateList> = ({ tab }) => {
  const ownedTemplateList = useSelector((state: IRootState) => state.minted.ownedTemplateList);
  const ownedReviewTemplateList = useSelector((state: IRootState) => state.minted.ownedReviewTemplateList);
  const ownedListedTemplateList = useSelector((state: IRootState) => state.minted.ownedListedTemplateList);
  ...
  switch (tab) {
            ...
            ownedTemplateList.map((template: ISelectedTemplate) => {
              ...
              return (
                <div key={template.token_id}>
                  <TemplateCard template={template} badge={filterTemplates(inReview, listed).badge} list={filterTemplates(inReview, listed).list} />
                </div>
              );
            })
          ) : (
    ...
    case TabType.REVIEW:
      return (
        <>
          {ownedReviewTemplateList.length > 0 ? (
            ...
            <NoTemplateDesign heading={'Sorry! No Review Templates!'} desc={'List a template to sell it on the Buildfy’s Marketplace'} />
          )}
        </>
      );
    case TabType.LISTED:
      return (
        <>
          {ownedListedTemplateList.length > 0 ? (
            ...
            <NoTemplateDesign heading={'Sorry! No Listed Templates!'} desc={'Sorry, you don’t have anything yet!'} />
          )}
        </>
      );
    default:

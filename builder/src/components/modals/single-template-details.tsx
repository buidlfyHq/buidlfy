...
const SingleTemplateDetails: FC<ISingleTemplateDetails> = ({ list }) => {
  ...
  const amount = selectedTemplateDto.buyoutPricePerToken && parseFloat(ethers.utils.formatUnits(selectedTemplateDto.buyoutPricePerToken));
    ...
    if (list) {
      ...
      if (selectedTemplateDto?.isOwned) {
        if (selectedTemplateDto?.value) {
          ...
          dispatch(updateWorkspaceBackgroundColor(selectedTemplateDto?.backgroundColor));
          dispatch(setSiteHead(selectedTemplateDto?.head));
          dispatch(updateContractAbi(JSON.stringify(selectedTemplateDto?.contract?.abi)));
          dispatch(updateContractAddress(selectedTemplateDto?.contract?.address));
          dispatch(updateContractNetwork(selectedTemplateDto?.contract?.network));
  ...
  return (
    <Dialog.Panel className="w-full max-w-[1200px] my-20 mx-28 rounded-[24px] py-10 px-14 bg-white">
      ...
      <div className="flex items-center justify-between">
        <div className="text-[22px] font-[500] text-[#14142B]">{selectedTemplateDto.name}</div>
      ...
      <div className="mt-5">
        <img src={selectedTemplateDto.image} className="w-full max-h-[509px] h-auto rounded-[28px]" alt="icon" height={669} />
        <div className="flex justify-between">
          <div className="mt-5">
            <div className="flex items-center mt-3">
              <div className="bg-[#9CB0D7] w-[32px] h-[32px] rounded-[50%] mt-30"> </div>
              <div className="ml-2 text-[14px] text-[#14142B] opacity-70">{truncateString(selectedTemplateDto.tokenOwner)}</div>
            </div>
            <div className="mt-8 text-[#4E4B66] opacity-70 text-[13px] max-w-[400px]">{selectedTemplateDto?.description}</div>
          </div>
          <div>
            {!list ? (
                ...
                {selectedTemplateDto?.isOwned ? (
                  ...
                  <>
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="text-[18px] font-[600] text-[#14142B]">{amount !== 0 ? `${amount} USDT` : 'Free'}</div>
                    </div>
                    <div className="text-[14px] font-[600] text-[#14142B] opacity-70">{amount !== 0 ? ` ~$ ${amount}` : null}</div>
                  </>
                )}
              </div>
            ) : (
            ...
            >
              <div className="text-[14px]">
                {!list ? (selectedTemplateDto?.isOwned ? 'Use Template' : 'Connect Wallet to Buy') : 'List on Buidlfy'}
              </div>
            ...
            <div className="flex mt-3 bg-[#F7F7FF] rounded-[8px] items-center text-[#14142B] opacity-80 text-[13px] py-3 px-4">
              ...
              <div className="ml-2">To buy this you must connect your wallet</div>
            </div>
          </div>
        </div>
      </div>
      <div className="flex gap-3 mt-7">{selectedTemplateDto?.category && <Tag name={selectedTemplateDto?.category} />}</div>
    </Dialog.Panel>
  );
};

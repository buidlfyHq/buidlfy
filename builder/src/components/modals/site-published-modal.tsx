...
const SitePublishedModal: FC = () => {
  ...
  return (
    <Dialog.Panel className="flex flex-col justify-center items-center w-full max-w-[580px] my-20 mx-28 rounded-[4px]  bg-white">
      <button className="flex items-start justify-end w-full pr-4 pt-4" onClick={() => dispatch(toggleModal(false))}>
      ...
      <div className="flex flex-col justify-center items-center py-16 px-10">
        ...
        <div className="font-[500] text-[20px] text-[#2C2D5E] mt-3">Site is published!</div>
        <div className="text-[13px] text-[#2C2D5E] font-[300] mt-2">Please open the link given below to see your site</div>
        <div className="flex items-center py-2 mt-5 another-bg link-bg px-7">
          <a target="_blank" rel="noreferrer" href={`https://${newDomainName}`} className="outline-none">
            {newDomainName}
          </a>

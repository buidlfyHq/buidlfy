...
interface MintTemplateModal {
    isMintTemplateOpen: boolean;
    setIsMintTemplateOpen: (isMintTemplateOpen: boolean) => void;
}

const MintTemplateModal : FC<MintTemplateModal> = ({
    isMintTemplateOpen,
    setIsMintTemplateOpen,
}) => {
    const [isMintedOpen, setIsMintedOpen] = useState<boolean>(false)
    return (
        <Dialog className="relative z-50" open={isMintTemplateOpen} onClose={() => setIsMintTemplateOpen(false)}>
                    ...
                    <Dialog.Panel className="flex flex-col w-full max-w-[800px] my-20 mx-28 rounded-[24px] py-12 px-10 bg-white min-h-full">
                        <div className="flex items-start justify-end w-full">
                            <div className="flex items-center justify-start w-full mt-8 cursor-pointer">
                                    ...
                                    <div className="ml-2 text-[13px] font-[500] text-[#4E4B66]">Back</div>
                            </div>
                        ...
                        <div className="my-8 mint-upload-img">
                            <div className="w-full h-[300px] upload-img-mint cursor-pointer">
                                <label htmlFor="inputTag" className="flex flex-col items-center justify-center h-full text-[12px] text-[#130F1C]">
                                    ...
                                    <span className="mt-5 text-[#651FFF] text-[12px]">Add from your desktop</span>
                                    <input
                        ...
                        <div className="mt-8">
                            ...
                            <input type="text" className="py-4 px-5 text-[13px] text-[#23314B] border border-[#C4C4C4] h-[45px] rounded-[8px] outline-none mt-2 w-full" placeholder="i.e. Crypto NFT Template"  />
                        </div>
                        <div className="mt-8">
                            <div className="text-[#23314B] text-[13px] font-[500]">Category</div>
                            <input type="text" className="py-4 px-5 text-[13px] text-[#23314B] border border-[#C4C4C4] h-[45px] rounded-[8px] outline-none mt-2 w-full" placeholder="Select Category"  />
                        </div>
                        <div className="mt-8">
                            <div className="text-[#23314B] text-[13px] font-[500]">Template Description</div>
                            <textarea rows={4} cols={50} className="py-4 px-5 text-[13px] text-[#23314B] border border-[#C4C4C4] rounded-[8px] outline-none mt-2 w-full" placeholder="i.e. Crypto NFT Template"></textarea>
                        </div>
                        <div className="flex items-center justify-end mt-12">
                            <div className="bg-[#E8E6EE] cursor-pointer text-[#7B7B7B] font-[500] text-[14px] py-2 px-10 rounded-[4px]">Cancel</div>
                            <div 

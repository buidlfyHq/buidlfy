...
function* getPublication({ payload }) {
  ...
  const fetchedPublication = yield call(
  ...
  if (!fetchedPublication.error) {
    try {
      ...
      const profilePicture =
        fetchedPublication?.publication?.profile.picture?.original?.url;
      console.log(profilePicture, "profilePicture");
      const coverPicture =
        fetchedPublication?.publication?.profile.coverPicture?.original?.url;
      ...
      const postMedia =
        fetchedPublication?.publication.metadata?.media[0]?.original?.url;

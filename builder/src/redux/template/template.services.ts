...
export const initiateTransactionService = async (listingId: BigNumber, buyoutPricePerToken: BigNumber) => {
  try {
    ...
    const tx = await marketplaceContract.buy(listingId, address, 1, config.address.usdt, buyoutPricePerToken, {
      gasLimit: 3000000,
    });

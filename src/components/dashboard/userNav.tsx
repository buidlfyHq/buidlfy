    ...
    return(
        <>
            {navbar === 0 ? (
                <div className='relative'>
                    <div className={`flex justify-evenly items-center h-[60px] border-b p-4 relative`} id="originalnav">
                        <div className="flex items-center justify-start ">
                            <SettingComponent classname={'mx-2 font-bold'} text={brandName} link={''} setBrandName={setBrandName} />
                        ...
                        <div className="flex items-center justify-center w-full">
                            ...
                            {menuArr.map((obj, index) => {
                                return(
                                    <div id={`${index}`}>
                                        <SettingComponent classname={'mx-1'} text={obj.name} link={obj.link} setMenuArr={setMenuArr} menuArr={menuArr} id={`${index}`} />
                                    </div>
                                )
                            })}
                            </div>
                        ...
                        <div className="flex items-center justify-end w-auto">
                            <div className="px-6 py-2 text-white bg-purple-400 cursor-pointer whitespace-nowrap rounded-xl">Connect Wallet</div>
                        </div>
                        {/* sudo */}
                        <div className={`flex justify-start items-center h-[60px] p-4 w-full absolute`} id="nav">
                            <div className='flex items-center px-3 py-2 bg-white border shadow-md cursor-pointer hover:text-purple-700' onClick={() => setNavbar(1)}>
                                <div>Delete</div>
                ...
                <div className="flex justify-evenly items-center h-[50px] border bg-white hover:text-purple-700 shadow-md m-2 py-2 px-3 w-[150px] cursor-pointer" onClick={() => setNavbar(0)}>
                    <div>Add Navbar</div>

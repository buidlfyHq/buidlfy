...
export default function SettingComponent({classname, text, link, setBrandName = null ,setMenuArr = null, id=null, menuArr=null}) {
  ...
  const handleTextChange = (e: any, Id:string) => {
    ...
    if(Id === null){setBrandName(e.target.value)}
    console.log(newMenuArr)
    let obj = 
    console.log(newMenuArr[parseInt(e.target.name)]) 
      ...
      <div className={`${classname} rounded-[8px] hover:bg-[#f5efef] py-2 px-4 cursor-pointer relative`} onClick={() => setOpen(true)}>
        ...
        <div className='border shadow-sm menu' ref={ref}>
          ...
          <div className='flex items-center px-3 mt-1'>
            ...
            <input name={Id} value={textVal} onChange={(e) => handleTextChange(e,Id)} className="changeText" type='text' placeholder='Name..' />
          </div>
          <div className='flex items-center px-3 mt-2'>
            <AiOutlineLink className='text-[18px] mr-3' />
            <input name={Id} value={linkVal} onChange={(e) => handleLinkChange(e,Id)} className="changeText" type='text' placeholder='URL...' />
          </div>
          <div className='h-[1px] w-full bg-gray-200 mt-3 mb-1'></div>
          <div className='flex items-center w-full px-3 py-2 text-gray-600 rounded cursor-pointer hover:bg-slate-100' onClick={(e) => deleteMenu(e,Id)}>
